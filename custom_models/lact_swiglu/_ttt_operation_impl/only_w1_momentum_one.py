...
def block_causal_lact_swiglu(
    ...
    Momentum (m_i) is always 1.
    no learning rate for w1.
    """
    del lr0, lr2 # make sure lr0 and lr2 are not used.
    has_momentum = momentum is not None
    assert has_momentum, "We expect momentum to be not None in this case."
    del momentum # make sure momentum is not used.
    del lr1 # make sure lr1 is not used.
    ...
    for i in range(0, seq_len - chunk_size, chunk_size):
        ...
        if has_momentum:
            m_i = 1.0 # momentum is always 1 in this case.
